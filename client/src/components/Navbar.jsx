...
const Navbar = () => {
  ...
  return (
    <>
      ...
      {/* SIDEBAR & OVERLAY - OUTSIDE NAVBAR */}
      <AnimatePresence>
        {open && (
          <>
            {/* Overlay */}
            ...
            <motion.div
              className={`
                fixed top-0 left-0 h-full w-4/5 max-w-xs z-[100]
                ${theme === "dark" ? "bg-gray-900" : "bg-white"}
                shadow-xl flex flex-col pt-20 px-6 gap-8 font-medium text-lg
                transition-colors
              `}
              initial="hidden"
              animate="visible"
            ...
            >
              {navLinks.map((link, i) => (
                ...
                >
                  <Link
                    to={link.path}
                    onClick={() => setOpen(false)}
                    className="block py-2 text-2xl font-medium text-gray-900 dark:text-gray-100 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
                  >
                    {link.label}
                  </Link>
                </motion.div>
              ...
              >
                <div className="relative">
                  ...
                  {/* Theme Menu */}
                  {themeMenuOpen && (
                    <div className="absolute left-0 mt-2 w-32 bg-white dark:bg-gray-800 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 py-1 z-50">
                      {["light", "dark", "system"].map((mode) => (
                        <button
                          key={mode}
                          onClick={() => handleThemeChange(mode)}
